...
| ⚠️ **Le démon ne démarre PLUS DU TOUT après avoir ajouté un backoff / un compteur de relance** (y compris sur une installation neuve) : `deamon_start()` appelle `deamon_info()`, donc un état écrit **avant** ce contrôle est relu par lui → marquer seulement **après** le contrôle `launchable`, juste avant l'`exec()`. Test qui l'attrape : **démarrage à froid** | `jeedom-dependances-et-demon.md` § 4 |
| ⚠️ **Un secret apparaît dans le log du démon alors que le code du plugin est propre** : les loggers de `python-roborock` propagent vers la racine et impriment un **corps HTTP brut** (INFO → `UserData` au login) et le **texte des exceptions** (ERROR → `local_key` via `homedata`) ; et brider un logger enfant **sans `max(racine, plafond)` AUGMENTE** la verbosité | `jeeroborock-cloud-api.md` § 9 |
